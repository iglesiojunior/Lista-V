...
def inicialização_vetor_num():
    ...
    if(opcao == 1):
        ...
        valor_min = 0
        valor_max = 0
        tamanho, valor_min, valor_max = map(int, input("\tInsira o tamanho e os valores min e max do vetor desejado:").split(" "))
...
def somar_valores_vetores(lista):
    resultado_soma = 0
    for i in range(0, len(lista)):
        resultado_soma += lista[i]
...
def atualizar_valores_lista(lista):
    ...
    option = int(input("\t>>>"))
    if(option == 1):
        lista_atualizada = vector_utils.(lista)
        option = int(input(""))
        return lista_atualizada
    if (option == 2):
        lista_atualizada = vector_utils.potenciacao_valores_lista(lista)
        return lista_atualizada
    if (option == 3):
        lista_atualizada = vector_utils.reduzir_valores_fracao(lista)
        return lista_atualizada
    if(option == 4):
